...
export default class FlatListCustom extends Component {
  ...
  render = () => {
    if (!this.state.data) throw new Error('No data found, please try again');
    ...
    return (
      <Animated.FlatList
        ...
        ListEmptyComponent={() =>
          this.props.isShowEmpty
            ? this.renderEmptyComponent(this.props.styleEmpty)
            : null
        }
        ...
        refreshControl={
          this.props.enableRefresh ? this.props.refreshControl : null
        }
        ...
        keyboardShouldPersistTaps="always" //The keyboard will not dismiss automatically, and the scroll view will not catch taps, but children of the scroll view can catch taps.
      />
    );
  };
    ...
    return (
      <View
        ...
        {this.props.isShowImageEmpty ? (
          <Image
            source={img_error_404}
            style={{
              marginTop: Constants.PADDING_XX_LARGE * 2,
  ...
  getParentData(props) {
    if (!props.data) throw new Error('No data found, please try again');
    let data = [];
    if (this.itemData === 1) {
      ...
      for (let i = 0, size = array.length / itemPerData; i < size; i++) {
        ...
        if (rowOrCol.length !== 0) data.push(rowOrCol);
      }
    }
    return data;
  }
  ...
  onLayoutChild(event) {
    ...
    if (childHeight > this.maxHeight) this.maxHeight = childHeight;
    if (childWidth > this.maxWidth) this.maxWidth = childWidth;
  }
    ...
    let viewPerRow = parentData.map((item, index) => {
      ...
      return (
        <View
          ...
          {this.props.renderItem(item, indexInData, parentIndex, index)}
        </View>
      );
    });
    return (
      <View style={styles.vertical}>
        ...
        {parentIndex !== this.state.data.length - 1
          ? this.props.renderParentLine
            ? this.props.renderParentLine(this.horizontal)
            : null
          : null}
      </View>
    );
