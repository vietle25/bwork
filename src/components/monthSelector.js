...
class MonthSelector extends React.Component {
    ...
    getSelectedBackgroundColor(month) {
        if (
            ...
            month.format(MONTH_YEAR_FORMAT) === this.props.selectedDate.format(MONTH_YEAR_FORMAT)
        ) {
    ...
    getSelectedForeGround(month) {
        ...
        if (month.format(MONTH_YEAR_FORMAT) === this.props.currentDate.format(MONTH_YEAR_FORMAT)) {
        ...
        return (
            <View
                ...
                <View style={styles.yearViewStyle}>
                    <Text style={this.props.yearTextStyle}>{this.state.initialView.format('YYYY')}</Text>
                </View>
    ...
    render() {
        ...
        const {containerStyle, swipable, velocityThreshold, directionalOffsetThreshold, gestureIsClickThreshold} =
            this.props;
        const SWIPE_CONFIG = {
            velocityThreshold,
...
MonthSelector.defaultProps = {
    ...
    seperatorHeight: 2,
    seperatorColor: Colors.COLOR_BACKGROUND,
    nextIcon: null,
    prevIcon: null,
    nextText: 'Next',
    prevText: 'Prev',
