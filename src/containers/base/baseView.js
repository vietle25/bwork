...
const resetActionLogin = CommonActions.reset({
    index: 0,
    routes: [{name: 'Login', params: {isExitApp: false}}],
});
...
class BaseView extends Component {
     ...
     */
    renderRightHeader = () => {
        return <View style={{padding: Constants.PADDING_12}} />;
    };
        ...
        return (
            <View style={{marginVertical: Constants.MARGIN_X_LARGE}}>
                <TouchableOpacity
                    ...
                    <Text style={[commonStyles.text, titleStyle, {fontSize: Fonts.FONT_SIZE_MEDIUM}]}>{title}</Text>
                </TouchableOpacity>
            </View>
        );
    }
    ...
    signInWithCustomToken(userId) {
        StorageUtil.retrieveItem(StorageUtil.FIREBASE_TOKEN)
            .then(firebaseToken => {
                    ...
                    Alert.alert(
                        ...
                        [
                            ...
                            },
                            {
                                text: 'OK',
                                onPress: () => {
                                    console.log('AAAAAAAAAAAA base view');
                                    global.isShowMessageError = false;
                                    // this.goLoginScreen();
                                },
                            },
        ...
        StorageUtil.retrieveItem(StorageUtil.MOBILE_CONFIG)
            .then(minute => {
                if (!Utils.isNull(minute)) {
                    ...
                    this.minuteBeforeCheckIn1 =
                        minute.find(x => x.name == 'minutes_before_working_time_able_to_check_in_1') || {};
                    this.minuteBeforeCheckIn2 =
                        minute.find(x => x.name == 'minutes_before_working_time_able_to_check_in_2') || {};
                    this.minuteAfterCheckIn1 =
                        minute.find(x => x.name == 'minutes_after_working_time_able_to_check_in_1') || {};
                    this.minuteAfterCheckIn2 =
                        minute.find(x => x.name == 'minutes_after_working_time_able_to_check_in_2') || {};

                    this.minuteBeforeCheckOut1 =
                        minute.find(x => x.name == 'minutes_before_working_time_able_to_check_out_1') || {};
                    this.minuteBeforeCheckOut2 =
                        minute.find(x => x.name == 'minutes_before_working_time_able_to_check_out_2') || {};
                    this.minuteAfterCheckOut1 =
                        minute.find(x => x.name == 'minutes_after_working_time_able_to_check_out_1') || {};
                    this.minuteAfterCheckOut2 =
                        minute.find(x => x.name == 'minutes_after_working_time_able_to_check_out_2') || {};
                }
            })
            .catch(error => {
                //this callback is executed when your Promise is rejected
    ...
    async getToken() {
        ...
        if (!Utils.isNull(fcmToken)) {
            let fcmTokenNew = await messaging().getToken();
