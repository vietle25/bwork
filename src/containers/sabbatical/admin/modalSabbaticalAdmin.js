...
export default class ModalSabbaticalAdmin extends BaseView {
    ...
    render() {
        ...
        let parseItem = {
            ...
            offFromDate: DateUtil.convertFromFormatToFormat(
                contentValue.offFromDate,
                DateUtil.FORMAT_DATE_SQL,
                DateUtil.FORMAT_DATE,
            ),
            offToDate: !Utils.isNull(contentValue.offToDate)
                ? DateUtil.convertFromFormatToFormat(
                      contentValue.offToDate,
                      DateUtil.FORMAT_DATE_SQL,
                      DateUtil.FORMAT_DATE,
                  )
                : null,
                ...
                <View
                    ...
                    <View
                        ...
                        <Text
                            ...
                            {parseItem.approvalStatus != 0
                                ? localizes('sabbatical.contentSabbatical')
                                : this.props.isApproved
                                ? localizes('sabbatical.approvalSabbatical')
                                : localizes('sabbatical.deniedSabbatical')}
                        </Text>
                        <TouchableOpacity
                    ...
                    <ScrollView
                        ...
                        <Text
                            style={[
                                commonStyles.textBold,
                                {margin: 0, marginVertical: Constants.MARGIN_LARGE, fontSize: Fonts.FONT_SIZE_MEDIUM},
                            ]}>
                        ...
                        {(parseItem.approvalStatus == approvalStatusType.WAITING_FOR_APPROVAL ||
                            parseItem.approvalStatus == approvalStatusType.DENIED) &&
                        !this.props.isApproved ? (
                            <View>
                                <Text
                                    style={{marginVertical: Constants.MARGIN_LARGE, fontSize: Fonts.FONT_SIZE_MEDIUM}}>
                                    Lý do từ chối:
                                </Text>
                                ...
                                <TextInputCustom
                                    ...
                                    value={
                                        parseItem.approvalStatus != approvalStatusType.DENIED
                                            ? this.textReason
                                            : parseItem.deniedNote
                                    }
                                    onChangeText={text => {
                                        this.onChangeTextReason(text);
                                    }}
                                    inputNormalStyle={{
                                        maxHeight: Constants.MARGIN_LARGE_TITLE_ONE_BTN * 2,
                                        borderColor:
                                            !Utils.isNull(this.state.isAlert) && this.state.isAlert
                                                ? Colors.COLOR_RED
                                                : Colors.COLOR_GRAY,
                    ...
                    <View
                        ...
                        <TouchableOpacity
                            ...
                            <Text
                                ...
                                {' '}
                                {parseItem.approvalStatus != approvalStatusType.WAITING_FOR_APPROVAL
                                    ? localizes('sabbatical.close')
                                    : localizes('sabbatical.cancel')}
                            </Text>
                        </TouchableOpacity>
                        {parseItem.approvalStatus == approvalStatusType.WAITING_FOR_APPROVAL ? (
                            <TouchableOpacity
    ...
    renderDate = (offFromDate, offToDate) => {
        if (!Utils.isNull(offFromDate) && !Utils.isNull(offToDate)) {
            return (
                <Text style={([styles.text], {fontSize: Fonts.FONT_SIZE_MEDIUM})}>
                    {'Từ ' + offFromDate + ' - ' + offToDate}
                </Text>
            );
        } else if (!Utils.isNull(offFromDate) && Utils.isNull(offToDate)) {
            return <Text style={([styles.text], {fontSize: Fonts.FONT_SIZE_MEDIUM})}>{'Ngày ' + offFromDate}</Text>;
        }
    };
