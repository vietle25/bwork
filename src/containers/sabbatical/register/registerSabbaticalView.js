...
class RegisterSabbaticalView extends BaseView {
    ...
    componentDidMount() {
        ...
        if (!Utils.isNull(this.data)) {
            ...
            state.offFromDate = DateUtil.convertFromFormatToFormat(
                this.data.offFromDate,
                DateUtil.FORMAT_DATE_SQL,
                DateUtil.FORMAT_DATE,
            );
            state.offToDate = DateUtil.convertFromFormatToFormat(
                this.data.offToDate,
                DateUtil.FORMAT_DATE_SQL,
                DateUtil.FORMAT_DATE,
            );
        ...
        if (this.props.errorCode != ErrorCode.ERROR_INIT) {
            if (this.props.errorCode == ErrorCode.ERROR_SUCCESS) {
                ...
                if (this.props.action == getActionSuccess(ActionEvent.GET_WORKING_TIME_CONFIG)) {
                        ...
                        if (data.shiftType == workingTimeShiftType.FULL_WORKING_DAY) {
                            this.workingTimes = [
                                ...
                                },
                                {
                                    ...
                                    name:
                                        this.formatTime(data.startWorkingTime1) +
                                        ' đến ' +
                                        this.formatTime(data.endWorkingTime1),
                                },
                                {
                                    id: sabbaticalOffType.FULL_WORKING_DAY_2,
                                    name:
                                        this.formatTime(data.startWorkingTime2) +
                                        ' đến ' +
                                        this.formatTime(data.endWorkingTime2),
                                },
                            ];
                    ...
                    this.props.action == getActionSuccess(ActionEvent.REGISTER_SABBATICAL) ||
    ...
    render() {
        ...
        let toDay = DateUtil.convertFromFormatToFormat(
            DateUtil.convertFromFormatToFormat(DateUtil.now(), DateUtil.FORMAT_DATE_TIME_ZONE_T, DateUtil.FORMAT_DATE),
            DateUtil.FORMAT_DATE,
            DateUtil.FORMAT_DATE_TIME_ZONE_T,
        );
        let offFromDateInput = Utils.validateDate(offFromDate)
            ? DateUtil.convertFromFormatToFormat(offFromDate, DateUtil.FORMAT_DATE, DateUtil.FORMAT_DATE_TIME_ZONE_T)
            : null;
        let offToDateInput = Utils.validateDate(offToDate)
            ? DateUtil.convertFromFormatToFormat(offToDate, DateUtil.FORMAT_DATE, DateUtil.FORMAT_DATE_TIME_ZONE_T)
            : null;
        this.toDayTimeStamp = DateUtil.getTimestamp(toDay);
        this.offFromDateTimeStamp =
            !Utils.isNull(offFromDateInput) && offFromDateInput != 'Invalid date'
                ? DateUtil.getTimestamp(offFromDateInput)
                : 0;
        this.offToDateTimeStamp =
            !Utils.isNull(offToDateInput) && offToDateInput != 'Invalid date'
    ...
    renderMenuWorkingTime = () => {
        return (
            <Menu style={{}} ref={ref => (this.menuOptionWorkingTime = ref)}>
                <MenuTrigger text="" />
                <MenuOptions>
    ...
    chooseDate = day => {
        let state = this.state;
        let date = DateUtil.convertFromFormatToFormat(day, DateUtil.FORMAT_DATE_TIME_ZONE_T, DateUtil.FORMAT_DATE);
