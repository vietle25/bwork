...
class TimekeepingHistoryDetailView extends BaseView {
    ...
    constructor(props) {
        ...
        this.createdAt = DateUtil.convertFromFormatToFormat(createdAt, DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DATE_SQL);
        this.userId = userId;
    ...
    renderProcessCheck() {
        return (
            <View style={[styles.boxProcess, { justifyContent: 'flex-start' }]}>
                ...
                <View style={[commonStyles.viewHorizontal, { flex: 0 }]}>
                    <Text style={[commonStyles.text, { flex: 1, marginHorizontal: 0 }]}>{localizes("timekeepingHistoryDetail.workingHours")}</Text>
                    <Text style={[commonStyles.text, { marginHorizontal: 0 }]}>
                        {DateUtil.parseMillisecondToHour(Math.round(this.realWorkingHours / 1000 / 60) * 1000 * 60)}
                    </Text>
                </View>
                <View style={[commonStyles.viewHorizontal, { flex: 0 }]}>
                    <Text style={[commonStyles.text, { flex: 1, marginHorizontal: 0 }]}>{localizes("timekeepingHistoryDetail.lackTime")}</Text>
                    <Text style={[commonStyles.text, { marginHorizontal: 0 }]}>
                        {DateUtil.parseMillisecondToHour(Math.round(this.deficientWorkingHours / 1000 / 60) * 1000 * 60)}
                    </Text>
                </View>
                <View style={[commonStyles.viewHorizontal, { flex: 0 }]}>
                    <Text style={[commonStyles.text, { flex: 1, marginHorizontal: 0 }]}>{localizes("timekeepingHistoryDetail.lateWorkingHours")}</Text>
                    <Text style={[commonStyles.text, { marginHorizontal: 0 }]}>
                        {DateUtil.parseMillisecondToHour(Math.round(this.lateWorkingHours / 1000 / 60) * 1000 * 60)}
                    </Text>
                </View>
                <View style={[commonStyles.viewHorizontal, { flex: 0 }]}>
                    <Text style={[commonStyles.text, { flex: 1, marginHorizontal: 0 }]}>{localizes("timekeepingHistoryDetail.detailCheck")}</Text>
                </View>
                <FlatListCustom
                    horizontal={false}
