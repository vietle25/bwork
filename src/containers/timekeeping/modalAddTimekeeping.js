...
class ModalAddTimekeeping extends BaseView {
    ...
    showModal = (data, user, workingTimeConfig, timekeepingRecord) => {
        ...
        let checkinWifiId = !Utils.isNull(data)
            ? !Utils.isNull(data.checkInWiFiConfig)
                ? data.checkInWiFiConfig.id
                : null
            : null;
        let checkoutWifiId = !Utils.isNull(data)
            ? !Utils.isNull(data.checkOutWiFiConfig)
                ? data.checkOutWiFiConfig.id
                : null
            : null;
        this.setState({
            isVisible: true,
    ...
    getCheckinTime = (timekeeping, timekeepingRecord, workingTimeConfig) => {
        ...
        if (!Utils.isNull(timekeeping)) {
            ...
            if (timekeepingRecord.length > 0) {
                if (
                    !this.isDuplicateTimekeeping(
                        workingTimeConfig.startWorkingTime1,
                        workingTimeConfig.endWorkingTime1,
                        timekeepingRecord,
                    )
                ) {
                    checkInTime = DateUtil.convertFromFormatToFormat(
                        workingTimeConfig.startWorkingTime1,
                    ...
                    if (
                        !this.isDuplicateTimekeeping(
                            workingTimeConfig.startWorkingTime2,
                            workingTimeConfig.endWorkingTime2,
                            timekeepingRecord,
                        )
                    ) {
                        checkInTime = DateUtil.convertFromFormatToFormat(
        ...
        if (!Utils.isNull(timekeeping)) {
            ...
            if (timekeepingRecord.length > 0) {
                if (
                    ...
                    checkOutTime = DateUtil.convertFromFormatToFormat(
                        workingTimeConfig.endWorkingTime1,
                        DateUtil.FORMAT_TIME_SECONDS,
                        DateUtil.FORMAT_TIME,
                    );
                        ...
                        <View
                            ...
                            <Text
                                style={[
                                    ...
                                    {flex: 1, color: Colors.COLOR_PRIMARY, margin: Constants.MARGIN_X_LARGE},
                                ]}>
                                {'THÊM CHẤM CÔNG'}
                            </Text>
                            <TouchableOpacity
                        ...
                        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="always">
                            ...
                            <View style={{marginHorizontal: Constants.MARGIN_X_LARGE}}>
                                <View style={{flexDirection: 'row'}}>
                                    ...
                                    <View style={{flex: 1, marginLeft: Constants.MARGIN_LARGE}}>
                                        <Text style={[styles.text]}>Wifi checkin</Text>
                                        {this.renderDropdownWifi('modalDropdownCheckin', checkinWifiDefault, {
                                            color: !Utils.isNull(this.state.checkinWifiId)
                                                ? Colors.COLOR_TEXT
                                                : Colors.COLOR_DRK_GREY,
                                        })}
                                    </View>
                                </View>
                                <View style={{paddingVertical: Constants.MARGIN_LARGE + Constants.MARGIN}}>
                                    <Text style={[styles.text]}>Lý do checkin</Text>
                                    <TextInputCustom
                                        inputNormalStyle={{
                                            paddingHorizontal: 0,
                                        }}
                            ...
                            <View style={{marginHorizontal: Constants.MARGIN_X_LARGE}}>
                                <View style={{flexDirection: 'row'}}>
                                    ...
                                    {/* Wifi checkout */}
                                    <View style={{flex: 1, marginLeft: Constants.MARGIN_LARGE}}>
                                        <Text style={[styles.text]}>Wifi checkout</Text>
                                        {this.renderDropdownWifi('modalDropdownCheckout', checkoutWifiDefault, {
                                            color: !Utils.isNull(this.state.checkoutWifiId)
                                                ? Colors.COLOR_TEXT
                                                : Colors.COLOR_DRK_GREY,
                                        })}
                                    </View>
                                ...
                                <View style={{paddingTop: Constants.MARGIN_LARGE + Constants.MARGIN}}>
                                    <Text style={[styles.text]}>Lý do checkout</Text>
                                    <TextInputCustom
                                        inputNormalStyle={{
                                            paddingHorizontal: 0,
                                        }}
                            ...
                            <View
                                ...
                                <TouchableOpacity
                                    ...
                                    onPress={() => this.hideModal()}>
                                    <Text style={[commonStyles.text, {marginVertical: 0}]}>{localizes('cancel')}</Text>
                                </TouchableOpacity>
                                <TouchableOpacity
                                    block
                                    activeOpacity={Constants.ACTIVE_OPACITY}
    ...
    onClickAddTimekeeping = () => {
        ...
        if (Utils.isNull(checkinTime)) {
            ...
            checkinTimestamp < start1 - minuteBeforeCheckIn1 ||
            (checkinTimestamp >= end1 + minuteAfterCheckOut1 && checkinTimestamp < start2 - minuteBeforeCheckIn2) ||
            checkinTimestamp > end2
        ) {
    ...
    isDuplicateTimekeeping = (checkinTime, checkoutTime, timekeepingRecord) => {
        ...
        for (let index = 0; index < timekeepingRecord.length; index++) {
            ...
            if (
                (checkinTime >= item.checkInTime && checkinTime <= item.checkOutTime) ||
