...
class SalaryHistoryView extends BaseView {
    ...
    constructor(props) {
        ...
        this.user = !Utils.isNull(user) && !Utils.isNull(this.screenType) && this.screenType == screenType.FROM_STAFF_SALARY ? user : null;
                    ...
                    >
                            ...
                            >
                                <View style={{ flex: 1 }}>
                                    <Text style={styles.textPeriod}>
                                        {`${localizes("salaryHistory.month")} ` + DateUtil.convertFromFormatToFormat(monthCurrentSQL, DateUtil.FORMAT_MONTH_OF_YEAR, DateUtil.FORMAT_MONTH)}
                        ...
                        <View style={styles.detailSalary}>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: Constants.MARGIN_12 }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRightGreen}>{salary.salary > 0 ? StringUtil.formatStringCash(salary.salary) + salary.inputType : "-"}</Text>
                                </View>
                            </View>
                            {/* numDayOffInMonth */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{!Utils.isNull(salary.numDayOffInMonth) ? salary.numDayOffInMonth : 0}</Text>
                                </View>
                            </View>
                            {/* numDayOffSabbatical */}
                            <View style={{ flexDirection: 'row' }}>
                                <View style={[styles.viewLeft, { flex: 1 }]}>
                                    <Text style={styles.textLeft}>{localizes("salaryHistory.numDayOffSabbatical")}</Text>
                                </View>
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{!Utils.isNull(salary.numDaySabbatical) ? salary.numDaySabbatical : 0}</Text>
                                </View>
                            </View>
                            {/* numLateForWork */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{!Utils.isNull(salary.numLateForWork) ? salary.numLateForWork : 0}</Text>
                                </View>
                            </View>
                            {/* workingDays */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{!Utils.isNull(salary.totalWorkdays) ? salary.totalWorkdays : 0}</Text>
                                </View>
                            </View>
                            {/* planWorkingHours */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{DateUtil.parseMillisecondToHour(Math.round(salary.totalPlanWorkingHours * 60) * 1000 * 60)}</Text>
                                </View>
                            </View>
                            {/* workingHours */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{DateUtil.parseMillisecondToHour(Math.round(salary.totalWorkingHours * 60) * 1000 * 60)}</Text>
                                </View>
                            </View>
                            {/* lackTime */}
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRight}>{DateUtil.parseMillisecondToHour(Math.round(salary.totalLackTime * 60) * 1000 * 60)}</Text>
                                </View>
                            </View>
                            {/* temporaryAmount */}
                            <View style={{ flexDirection: 'row' }}>
                                <View style={[styles.viewLeft, { flex: 1 }]}>
                                    <Text style={styles.textLeft}>
                                        ...
                                        {
                                            DateUtil.convertFromFormatToFormat(this.dayOfToDay, DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DAY) == this.firstDay ||
                                                this.filter.month !=
                                                DateUtil.convertFromFormatToFormat(DateUtil.now(), DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DATE_MONTH_OF_YEAR)
                                                ? null
                                                : `${localizes("salaryHistory.to")} ${DateUtil.convertFromFormatToFormat(today, DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DATE)
                                                }`
                                        }
                                    </Text>
                                </View>
                                <View style={{ flex: 0 }}>
                                    <Text style={styles.textRightGreen}>{salary.totalTemporaryAmount >= 0
                                        ? StringUtil.formatStringCash(Math.ceil(salary.totalTemporaryAmount))
                                        : "- " + StringUtil.formatStringCash(Math.ceil(Math.abs(salary.totalTemporaryAmount)))}
                                    </Text>
                                </View>
                            </View>
                        </View>
                        ...
                        <View style={styles.totalSalary}>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: Constants.MARGIN_12 }}>
                                <View style={{ flex: 1 }[styles.viewLeft]}>
                                    <TouchableOpacity
                                        activeOpacity={Constants.ACTIVE_OPACITY}
                                        onPress={() => this.props.navigation.navigate('SalaryHistoryDetail', {
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    >
                                        <Text style={styles.textRightGreen}>{StringUtil.formatStringCash(Math.ceil(salary.totalBonusAmount))}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                            {/* Total Fine */}
                            ...
                            <View style={{ flexDirection: 'row' }}>
                                ...
                                <View style={{ flex: 1 }}>
                                    ...
                                    >
                                        <Text style={styles.textRightGreen}>{salary.totalFineAmount != 0 ? '- ' + StringUtil.formatStringCash(Math.ceil(salary.totalFineAmount * (-1))) : '0 VND'}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                            <View>
                                    ...
                                    <View style={{ flex: 1 }}>
                                        <Text style={styles.textRightGreen}>
                                            {!Utils.isNull(salary.netAmount)
                                                && this.filter.month != DateUtil.convertFromFormatToFormat(DateUtil.now(), DateUtil.FORMAT_DATE_TIME_ZONE, DateUtil.FORMAT_DATE_MONTH_OF_YEAR)
