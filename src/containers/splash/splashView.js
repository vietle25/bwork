...
class SplashView extends BaseView {
    ...
    componentDidMount() {
        DeviceInfo.getUniqueId().then(id => (global.deviceId = id));
    ...
    validateCompany() {
        ...
        if (user.company.id != 1) {
            ...
            if (!Utils.isNull(companyInfo)) {
                if (
                    !Utils.isNull(companyInfo.company) &&
                    (!Utils.isNull(companyInfo.branch) || Utils.isNull(companyInfo.company.branches))
                ) {
                    this.goHomeScreen();
                } else {
     ...
     */
    getToken() {
        StorageUtil.retrieveItem(StorageUtil.USER_TOKEN)
            .then(token => {
                global.token = token;
                this.login();
            })
