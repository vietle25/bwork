...
class RegisterPartnerView extends BaseView {
    ...
    */
    ...
    render() {
        ...
        return (
            <Container style={styles.container}>
                <Root>
                    <BackgroundTopView ratio={this.screen == screenType.FROM_USER_PROFILE ? Constants.RATIO_BACKGROUND_TOP_VIEW : 3} />
                    <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 }}>
                        <Header style={[{ backgroundColor: Colors.COLOR_TRANSPARENT, borderBottomWidth: 0, elevation: 0 }]}>
                            <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                                {this.renderHeaderView({
                                    visibleBack: this.screen == screenType.FROM_USER_PROFILE
                                        ? true
                                ...
                                {this.screen == screenType.FROM_USER_PROFILE
                                    ...
                                    : <TouchableOpacity activeOpacity={Constants.ACTIVE_OPACITY} onPress={this.confirmAction} >
                                        ...
                                        { fontSize: Fonts.FONT_SIZE_MEDIUM, marginRight: Constants.MARGIN_LARGE, color: Colors.COLOR_WHITE }
                                        ]}>{nextText}</Text>
                                    </TouchableOpacity>}
                            </View>
                        ...
                        <Content
                            ...
                            <View style={{ flexGrow: 1 }}>
                                {this.screen == screenType.FROM_USER_PROFILE ?
                                    null : <View style={[commonStyles.viewCenter]}>
    ...
    renderListPartner() {
        return (
            <View style={{ marginHorizontal: Constants.MARGIN_X_LARGE, marginTop: this.screen == screenType.FROM_USER_PROFILE ? 0 : Constants.MARGIN_XX_LARGE }}>
                <BackgroundShadow
                        ...
                        <View>
                            <FlatListCustom
                                style={{
                                    ...
                                    borderBottomLeftRadius: this.screen == screenType.FROM_USER_PROFILE ? 0 : Constants.CORNER_RADIUS,
                                    borderBottomRightRadius: this.screen == screenType.FROM_USER_PROFILE ? 0 : Constants.CORNER_RADIUS,
                            ...
                            />
                            {/* Button save */}
                            {this.screen == screenType.FROM_USER_PROFILE ? <TouchableOpacity
                                activeOpacity={Constants.ACTIVE_OPACITY}
                                onPress={this.confirmAction} block
    ...
    renderItem(item, index, parentIndex, indexInParent) {
        ...
        return (
            <View>
                ...
                />
                {/* HR */}
                {index != this.listPartner
                    .length - 1 ? <Hr style={{ marginHorizontal: Constants.MARGIN_X_LARGE }} color={Colors.COLOR_BACKGROUND} />
                    : this.screen == screenType.FROM_USER_PROFILE
    ...
    onItemSelected(item, index, isNextChild = true) {
        ...
        if (!Utils.isNull(item.child)) {
            ...
            this.setState({
                nextText: this.screen == screenType.FROM_USER_PROFILE
                    ? localizes('registerPartner.save') : localizes('registerPartner.confirm'),
                next: false,
                data: item
            })
        }
