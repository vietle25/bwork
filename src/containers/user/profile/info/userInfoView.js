...
class UserInfoView extends BaseView {
    constructor(props) {
        super(props);
        const {userInfo, callBack, screen} = this.props.route.params;
        this.state = {
            enableRefresh: false,
            refreshing: true,
                ...
                <MenuOptions>
                    ...
                    <MenuOption
                        ...
                        <View
                            ...
                            <Text style={[commonStyles.text, {flex: 1}]}>Cấu hình</Text>
                        </View>
                    </MenuOption>
                </MenuOptions>
            </Menu>
                ...
                <View style={{flex: 1}}>
                    <HStack style={[commonStyles.header]}>
                        {this.renderHeaderView({
                            ...
                            visibleBack:
                                this.screen == screenType.FROM_SPLASH || this.screen == screenType.FROM_LOGIN
                                    ? false
                                    : true,
                        ...
                        <View
                            ...
                            <View>
                                <TextInputCustom
                                    ...
                                    onPress={() => {
                                        this.state.visibleEdit ? this.menuOptionGender.open() : null;
                                    }}
                                    isInputAction={true}
                                    ...
                                    contentRight={this.state.visibleEdit ? <Image source={ic_down_grey} /> : null}
                                    editable={this.state.visibleEdit}
                                />
                            ...
                            <TextInputCustom
                                ...
                                onChangeText={personalId => this.setState({personalId: personalId})}
    ...
    getUserAvatarPath = avatarPath => {
        return !Utils.isNull(avatarPath) && avatarPath.indexOf('http') != -1
            ? avatarPath
            : this.resourceUrlPath.textValue + '/' + global.companyIdAlias + '/' + avatarPath;
    };
     ...
     */
    onEditData = () => {
        const {phone, fullName, dayOfBirth, address, email, gender, personalId, domicile, avatarPath} = this.state;
        let editData = {
    ...
    uploadPersonalIdImages() {
        ...
        if (!Utils.isNull(this.state.personalImages[this.indexImage])) {
            ...
            const options = {
                url:
                    ServerPath.API_URL + `user/personal/${this.state.personalImages[this.indexImage].imageSide}/upload`,
                path: filePathUrl,
                method: 'POST',
                field: 'file',
                type: 'multipart',
     ...
     */
    validate() {
        const {avatarPath, fullName, email, phone, personalId, domicile, address, personalImages, dayOfBirth} =
            this.state;
        ...
        if (Utils.isNull(avatarPath)) {
            ...
            Utils.isNull(this.userResources) &&
    ...
    renderMenuGender = () => {
        return (
            <Menu style={{}} ref={ref => (this.menuOptionGender = ref)}>
                <MenuTrigger text="" />
                <MenuOptions>
    ...
    addFaceToPerson = personId => {
        ...
        let avatar =
            !Utils.isNull(avatarPath) && avatarPath.indexOf('http') != -1
                ? avatarPath
                : this.resourceUrlPath.textValue + '/' + global.companyIdAlias + '/' + avatarPath;
        let body = {
            url: avatar,
        ...
        fetch(
            ServerPath.FACE_API +
                `persongroups/${PERSON_GROUP_ID}/persons/${personId}/persistedFaces?userData=${fullName}&detectionModel=detection_02`,
            {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
    ...
    detectFace = type => {
        ...
        fetch(
            ServerPath.FACE_API +
                `detect?returnFaceId=true&recognitionModel=recognition_02&returnRecognitionModel=true&detectionModel=detection_02`,
            {
                method: 'POST',
                headers: {
                    Accept: 'application/json',
            ...
            .then(responseJson => {
                ...
                if (responseJson.error != null) {
                    ...
                    if (responseJson[0].candidates.length > 0) {
                        ...
                        if (
                            responseJson[0].candidates[0].confidence >= 0.7 &&
