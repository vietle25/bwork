...
import { View, Text, Image, TouchableOpacity, ImageBackground, StyleSheet, Dimensions } from 'react-native';
...
export default class ItemTimekeeping extends PureComponent {
    ...
    render() {
        const { data, index, screen, dashboardType, renderButtonAproval, renderButtonEdit } = this.props;
        const CheckIn = ({ textCheck, time, isNote, styleNote, type }) => (
            <View style={{ position: 'relative' }}>
                ...
                <View style={{ position: 'relative', flexDirection: 'row', alignItems: 'center' }}>
                    <View style={[styles.circleCheck, { borderColor: isNote ? Colors.COLOR_TRANSPARENT : Colors.COLOR_PRIMARY }]} />
                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                        ...
                        <Text style={[commonStyles.text, { flex: 1, margin: 0 }, isNote ? styleNote : null]}>{textCheck}
                            {isNote
                    ...
                    {isNote
                        && ((type == checkInType.CHECK_IN && this.data.checkInApprovalStatus == approvalStatusType.WAITING_FOR_APPROVAL)
                            || (type == checkInType.CHECK_OUT && this.data.checkOutApprovalStatus == approvalStatusType.WAITING_FOR_APPROVAL))
                        && renderButtonAproval && renderButtonAproval(this.data, type)}
                </View>
        ...
        const CheckOut = ({ textCheck }) => (
            <View>
                ...
                <CheckIn
                    textCheck={textCheck}
                    time={DateUtil.convertFromFormatToFormat(this.data.checkOutTime, DateUtil.FORMAT_TIME_SECOND, DateUtil.FORMAT_TIME)}
                />
        ...
        return (
            <View style={{
                ...
                paddingHorizontal: screen == screenType.FROM_HOME_VIEW
                    ? Constants.PADDING_X_LARGE + Constants.PADDING_LARGE
                    : 0
            }}>
                <View style={{ flex: 1 }}>
                    <CheckIn
                        textCheck={!Utils.isNull(this.data.id) ? localizes("timekeeping.checkedIn") : "Chưa checkin"}
                        time={DateUtil.convertFromFormatToFormat(this.data.checkInTime, DateUtil.FORMAT_TIME_SECOND, DateUtil.FORMAT_TIME)}
                    />
                    {this.data.checkInType != timekeepingType.NORMAL && !Utils.isNull(this.data.checkInNote)
                        ...
                        : null}
                    {!Utils.isNull(this.data.checkOutTime)
                        ? <CheckOut textCheck={!Utils.isNull(this.data.id) ? localizes("timekeeping.checkedOut") : "Chưa checkout"} />
                        : null
                    }
                ...
                {(this.data.checkInApprovalStatus !== approvalStatusType.WAITING_FOR_APPROVAL)
                    ...
                    && renderButtonEdit && renderButtonEdit(this.data)}
            </View>
        );
    }
    ...
    getTextApproval(approvalStatus, type) {
        if (approvalStatus == approvalStatusType.WAITING_FOR_APPROVAL) {
            return <Text style={[commonStyles.text, { lineHeight: 24, color: Colors.COLOR_RED }]}>{localizes("timekeeping.waitingApproval")}</Text>
        } else if (approvalStatus == approvalStatusType.DENIED) {
            return <Text style={[commonStyles.text, { lineHeight: 24, color: Colors.COLOR_RED }]}>{localizes("timekeeping.denied")}</Text>
    ...
    getTextType(type) {
        if (type != timekeepingType.NORMAL) {
            return <Text style={[commonStyles.text, { lineHeight: 24, color: Colors.COLOR_PRIMARY }]}>{localizes("timekeeping.approved")}</Text>
        } else {
            return null
        }
    }
