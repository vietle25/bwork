...
class HomeView extends GeoLocationView {
    constructor(props) {
        ...
        global.toggleAlarm = this.toggleAlarm.bind(this);
                ...
                if (this.props.action == getActionSuccess(ActionEvent.GET_UPDATE_VERSION)) {
                    ...
                    if (!Utils.isNull(data)) {
                        if (!Utils.isNull(data.today)) {
                            ...
                            this.todayTimeStamp = DateUtil.getTimestamp(DateUtil.convertFromFormatToFormat(
                                data.today,
                                DateUtil.FORMAT_DATE_TIME_ZONE,
                        ...
                        if (!Utils.isNull(data.timekeepingRecord)) {
                            ...
                            data.timekeepingRecord.forEach((item, index) => {
                                if (Utils.isNull(item.checkOutTime)) {
                                    let timeCheckIn = DateUtil.getTimestamp(DateUtil.convertFromFormatToFormat(
                                        this.todaySQL + " " + item.checkInTime, DateUtil.FORMAT_DATE_TIMES, DateUtil.FORMAT_DATE_TIME_ZONE_T));
    ...
    handleAlarm() {
        ...
        let minuteAlarmCheckIn1 = !Utils.isNull(this.alarmBeforeCheckIn1.numericValue) ? this.alarmBeforeCheckIn1.numericValue : 0;
        let minuteAlarmCheckOut1 = !Utils.isNull(this.alarmBeforeCheckOut1.numericValue) ? this.alarmBeforeCheckOut1.numericValue : 0;
        let minuteAlarmCheckIn2 = !Utils.isNull(this.alarmBeforeCheckIn2.numericValue) ? this.alarmBeforeCheckIn2.numericValue : 0;
        let minuteAlarmCheckOut2 = !Utils.isNull(this.alarmBeforeCheckOut2.numericValue) ? this.alarmBeforeCheckOut2.numericValue : 0;
        ...
        if (holidayTime.lastDayOfWeek < dayOffWeek.SATURDAY) {
            for (let index = 0; index < dayOffWeek.SATURDAY - holidayTime.lastDayOfWeek; index++) {
                dayOffs.push(...DateUtil.getAllDayByDayOfWeek(year, month, today, index + holidayTime.lastDayOfWeek + 1));
            }
        } else {
        ...
        holidayLeave.forEach(item => {
            let fromDate = DateUtil.convertFromFormatToFormat(item.fromDate + "/" + year, DateUtil.FORMAT_DATE, DateUtil.FORMAT_DATE_TIME_ZONE_T);
            let toDate = DateUtil.convertFromFormatToFormat(item.toDate + "/" + year, DateUtil.FORMAT_DATE, DateUtil.FORMAT_DATE_TIME_ZONE_T);
            ...
            if (numFromDateToDate > 0) {
                for (let index = 0; index <= numFromDateToDate; index++) {
                    ...
                    dayOffs.push(DateUtil.convertFromFormatToFormat(date, DateUtil.FORMAT_DATE_TIME_ZONE_T, "DD-MM-YYYY"));
                }
            } else {
                dayOffs.push(DateUtil.convertFromFormatToFormat(fromDate, DateUtil.FORMAT_DATE_TIME_ZONE_T, "DD-MM-YYYY"));
            }
        });
        ...
        if (!Utils.isNull(alarm)) {
            if (alarm.isOn) {
                ...
                let fromDate = DateUtil.convertFromFormatToFormat(alarmTime.day, DateUtil.FORMAT_DATE_SQL, DateUtil.FORMAT_DATE_TIME_ZONE_T);
                for (let index = 0; index <= dayOffWeek.SATURDAY; index++) {
                    let date = new Date(new Date(fromDate).setDate(new Date(fromDate).getDate() + index))
                    let dateSql = DateUtil.convertFromFormatToFormat(date, DateUtil.FORMAT_DATE_TIME_ZONE_T, DateUtil.FORMAT_DATE_SQL);
        ...
        days.forEach(item => {
            ...
            let dayTimeT = DateUtil.convertFromFormatToFormat(day, DateUtil.FORMAT_DATE_SQL, DateUtil.FORMAT_DATE_TIME_ZONE_T);
                ...
                if (workingOnSaturday.workingType == workingOnSaturdayType.RESTART_EACH_MONTH) {
                    ...
                    if (firstWorkingOnSaturdayWeekInMonth != DateUtil.getISOWeekInMonth(new Date(dayTimeT)).week) {
                        ...
                        saturdayOffs = saturdayOffs.filter(item => {
                            return DateUtil.getISOWeekInMonth(new Date(
                                DateUtil.convertFromFormatToFormat(item, "DD-MM-YYYY", DateUtil.FORMAT_DATE_TIME_ZONE_T)
                    ...
                    let numOfSaturday = DateUtil.convertFromFormatToFormat(firstSaturdayOn, DateUtil.FORMAT_DATE_SQL, DateUtil.FORMAT_DAY);
                    let saturdayOffs = [];
                    saturdayOffs.push(...DateUtil.getAllDayByDayOfWeek(year, month, numOfSaturday, dayOffWeek.SATURDAY));
            ...
            if (!dayOffs.includes(formatDay)) {
                if (!Utils.isNull(startWorkingTime1) && minuteAlarmCheckIn1 > 0) {
                    let alarmCheckIn1 = this.formatWorkingTime(startWorkingTime1, day)
                        - (minuteAlarmCheckIn1 * 1000 * 60);
                ...
                if (!Utils.isNull(endWorkingTime1) && minuteAlarmCheckOut1 > 0) {
                    let alarmCheckOut1 = this.formatWorkingTime(endWorkingTime1, day)
                        - (minuteAlarmCheckOut1 * 1000 * 60);
                ...
                if (!Utils.isNull(startWorkingTime2) && minuteAlarmCheckIn2 > 0) {
                    let alarmCheckIn2 = this.formatWorkingTime(startWorkingTime2, day)
                        - (minuteAlarmCheckIn2 * 1000 * 60);
                ...
                if (!Utils.isNull(endWorkingTime2) && minuteAlarmCheckOut2 > 0) {
                    let alarmCheckOut2 = this.formatWorkingTime(endWorkingTime2, day)
                        - (minuteAlarmCheckOut2 * 1000 * 60);
    ...
    isWiFiAllows() {
        ...
        if (this.wiFiListAllows.some(item => {
    ...
    isInWorkingTime(checkInTimeLastTimestamp) {
        ...
        if (!Utils.isNull(checkInTimeLastTimestamp)) {
            ...
            if ((checkInTimeLastTimestamp < end1 && this.todayTimeStamp <= (end1 + _59s))
                || (checkInTimeLastTimestamp < end2 && checkInTimeLastTimestamp >= start2 && this.todayTimeStamp <= (end2 + _59s))) {
                return true;
            } else {
                return false;
            }
            ...
            if ((start1 <= this.todayTimeStamp && this.todayTimeStamp < end1)
    ...
    isTimekeeping() {
        ...
        if (!Utils.isNull(checkOutTimeLastTimestamp)) {
            if (this.todayTimeStamp > checkOutTimeLastTimestamp) {
                ...
                let error = {
                    message: "todayTimeStamp: " + this.todayTimeStamp + ", checkOutTimeLastTimestamp: " + checkOutTimeLastTimestamp
    ...
    timekeeping() {
        ...
        if (Utils.isNull(this.wiFiListAllows)) {
            ...
            } else if (!this.isInWorkingTime(checkInTimeLastTimestamp) && this.filter.timekeepingType == timekeepingType.NORMAL) {
                if (checkingIn) {
    ...
    render() {
        ...
        if (!Utils.isNull(company)) {
            ...
            avatarCompany = hasHttp ? company.avatarPath : this.resourceUrlPathResize.textValue + "=" + global.companyIdAlias + "/" + company.avatarPath;
        }
        return (
            // <MyApp />
                    ...
                    >
                        ...
                        <TouchableOpacity
                            ...
                            {/* <ImageLoader
                                ...
                                path={avatarCompany}
                            /> */}
                            <Text numberOfLines={1} style={[commonStyles.text, { flex: 1, fontSize: Fonts.FONT_SIZE_X_MEDIUM }]}>
                        ...
                        <View style={styles.boxCheck}>
                            ...
                            >
                                ...
                                <View style={{ flex: 1, marginHorizontal: Constants.MARGIN_X_LARGE }}>
                                    <Text style={[commonStyles.text, { fontSize: Fonts.FONT_SIZE_SMALL, margin: 0, marginBottom: -2 }]}>{localizes("homeView.wifi")}</Text>
                                    <Text style={[commonStyles.text, { margin: 0, marginTop: -1 }]}>{!Utils.isNull(wiFiName) ? wiFiName : "-"}</Text>
                                </View>
                                <Image source={ic_down_grey} />
                            </TouchableOpacity>
                            <TouchableOpacity
                                activeOpacity={Constants.ACTIVE_OPACITY}
                                onPress={() => !this.props.isLoading && !refreshing
                                    ? this.handleRequestTimekeeping()
                                    : null
                                }
                                style={styles.buttonCheck}
                            >
                                <Text style={styles.textInCheck}>{checkingIn ? "TAN CA" : "VÀO LÀM"}</Text>
                                <Text style={[styles.textInCheck, {
                                    opacity: Constants.OPACITY_50
                                }]}>{timeCheckIn > 0 ? DateUtil.parseMillisecondToTime(timeCheckIn * 1000) : "00:00"}</Text>
                            </TouchableOpacity>
                            {/* <Text style={styles.textAddress}>{address}</Text> */}
                            {!Utils.isNull(this.workingTimeConfig)
                                ? <View style={{ width: "100%" }}>
                                    <Text style={[commonStyles.textBold]}>
                                        Ca làm việc: <Text style={[commonStyles.text]}>
                                    ...
                                    {this.workingTimeConfig.shiftType == workingTimeShiftType.FULL_WORKING_DAY
                                        ? <View style={{ width: "100%" }}>
                                            <View style={{ flexDirection: 'row', width: "100%" }}>
                                                <View style={{ flexDirection: 'row', flex: 1 }}>
                                                    {this.renderTextWorkingTime("Giờ bắt đầu:", this.workingTimeConfig.startWorkingTime1)}
                                                </View>
                                                <View style={{ flexDirection: 'row', flex: 1 }}>
                                                    {this.renderTextWorkingTime("Giờ nghỉ giữa ca:", this.workingTimeConfig.endWorkingTime1)}
                                                </View>
                                            </View>
                                            <View style={{ flexDirection: 'row', width: "100%" }}>
                                                <View style={{ flexDirection: 'row', flex: 1 }}>
                                                    {this.renderTextWorkingTime("Giờ bắt đầu lại:", this.workingTimeConfig.startWorkingTime2)}
                                                </View>
                                                <View style={{ flexDirection: 'row', flex: 1 }}>
                                                    {this.renderTextWorkingTime("Giờ kết thúc:", this.workingTimeConfig.endWorkingTime2)}
                                                </View>
                                            </View>
                                        </View>
                                        :
                                        <View style={{ flexDirection: 'row', width: "100%" }}>
                                            ...
                                            <View style={{ flexDirection: 'row', flex: 1 }}>
                                                {this.renderTextWorkingTime("Giờ kết thúc:", this.workingTimeConfig.endWorkingTime1)}
                                            </View>
                                        </View>
                                    }
                                </View>
                        ...
                        <View style={{ marginTop: Constants.MARGIN_LARGE, backgroundColor: Colors.COLOR_WHITE, paddingVertical: Constants.PADDING_LARGE }}>
                            <View style={[styles.boxDate]}>
                                <View style={{ height: "100%" }}>
                                    <Text style={styles.textDay}>
                                        {DateUtil.convertFromFormatToFormat(this.todaySQL, DateUtil.FORMAT_DATE_SQL, DateUtil.FORMAT_DAY)}
                                    </Text>
                                </View>
                            ...
                            <Hr width={2} style={{ marginHorizontal: Constants.MARGIN_X_LARGE + Constants.MARGIN_LARGE }} color={Colors.COLOR_BACKGROUND} />
                        </View>
    ...
    getMinutesRemindTimekeeping = () => {
        StorageUtil.retrieveItem(StorageUtil.MOBILE_CONFIG).then((alarm) => {
            if (!Utils.isNull(alarm)) {
                // console.log('alarm', alarm)
                this.alarmBeforeCheckIn1 = alarm.find(x => x.name == 'minutes_before_working_time_to_remind_check_in_1') || {}
                this.alarmBeforeCheckIn2 = alarm.find(x => x.name == 'minutes_before_working_time_to_remind_check_in_2') || {}
                this.alarmBeforeCheckOut1 = alarm.find(x => x.name == 'minutes_before_working_time_to_remind_check_out_1') || {}
                this.alarmBeforeCheckOut2 = alarm.find(x => x.name == 'minutes_before_working_time_to_remind_check_out_2') || {}
            }
        }).catch((error) => {
            //this callback is executed when your Promise is rejected
     ...
     */
    getHoliday = () => {
        StorageUtil.retrieveItem(StorageUtil.MOBILE_CONFIG).then((holiday) => {
